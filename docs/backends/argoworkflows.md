...
- **`Workflow`**: For one-time or manually triggered pipeline executions (when `schedules: null`)
- **`CronWorkflow`**: For scheduled, recurring pipeline executions (when `schedules` is a non-empty list)
...
!!! important "Generate-Time vs Runtime Configuration"
    ...
    - **Generate-Time (YAML)**: A `values.yaml` file configures the **workflow structure** — container images, namespaces, schedules, security contexts, resource requests/limits, and artifact storage. This is required when running `wurzel generate`.
...
# Or generate a one-time Workflow (set schedules: null in values.yaml)
wurzel generate --backend ArgoBackend \
...
workflows:
  pipelinedemo:
    ...
    schedules:
      - "0 4 * * *"  # Cron schedules for CronWorkflow, or null for one-time Workflow
    ...
    # Defaults to kubernetes.io/arch: amd64. Set nodeSelector: {} to clear it.
    nodeSelector:
    ...
    # Generated by default to harden Argo executor containers (wait/init).
    # Set a custom podSpecPatch only when you need to replace that generated patch.
    # podSpecPatch: |
    #   containers:
    #     - name: wait
    #       securityContext:
...
The Argo backend generates different workflow types based on the `schedules` configuration:

#### Normal Workflow (One-Time Execution)

Set `schedules: null` (or omit it) to create a **Workflow** for manual or one-time execution:

```yaml
workflows:
  my-workflow:
    name: my-pipeline
    schedules: null  # Creates a Workflow, not a CronWorkflow
```
...
Set one or more cron schedule strings to create a **CronWorkflow** for recurring execution:
...
workflows:
  my-cron-workflow:
    ...
    schedules:
      - "0 4 * * *"  # Creates a CronWorkflow that runs daily at 4 AM
```
...
!!! tip "Choosing the Right Type"
    - Use **Workflow** (`schedules: null`) when you need explicit control over when pipelines run
    - Use **CronWorkflow** (with a non-empty `schedules` list) for automated, time-based execution
...
| `nodeSelector` | map | `{"kubernetes.io/arch": "amd64"}` | Pod node selector. Set to `{}` to render no selector |
| `podSpecPatch` | string/null | generated executor patch | Custom pod spec patch override. By default Wurzel patches Argo's `wait` container and `init` initContainer with restricted security context and required resources |

...
def test_generate_dict(tmp_path: Path):
    ...
    assert all(key in dvc_pipe["StepImplementedLeaf"]
               for key in ("deps", "outs", "cmd"))
...
def test_generate_nested_dict(nested_steps):
    ...
    assert all(key in dvc_pipe[step2.__class__.__name__]
               for key in ("deps", "outs", "cmd"))
    assert all(key in dvc_pipe[step1.__class__.__name__]
               for key in ("deps", "outs", "cmd"))

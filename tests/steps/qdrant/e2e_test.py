...
from wurzel.steps.qdrant.data import QdrantMultiVectorResult, QdrantResult, TelemetryCollections, TelemetryResponse, \
    TelemetryResult
...
def test_qdrant_connector_first(input_output_folder: tuple[Path, Path], dummy_collection):
    ...
    mock_telemetry = TelemetryResponse(result=TelemetryResult(collections=TelemetryCollections(collections=[])))

    with unittest.mock.patch("wurzel.steps.qdrant.step.QdrantConnectorStep._get_telemetry",
...
def test_qdrant_collection_retirement(input_output_folder: tuple[Path, Path], env, dummy_collection):
    ...
    def mock_get_telemetry(*args, **kwargs):
        # Emulate 4 collections — one will be aliased, one recently used, one old and deletable
        return {
            "result": {
                "collections": {
                    "collections": [
                        {"id": "austria_v1",
                         "shards": [{"local": {"optimizations": {"optimizations": {"last_responded": old_time}}}}]},
                        {"id": "austria_v2", "shards": [{"remote": [{"searches": {"last_responded": old_time}}]}]},
                        {"id": "austria_v3", "shards": [{"remote": [{"searches": {"last_responded": recent_time}}]}]},
                        {"id": "austria_v4", "shards": [],
                         },
                    ]
                }
            }
        ...
        client.get_telemetry = mock_get_telemetry

        # Simulate four executions to create 4 collections
        ...
        collection_names = [col.name for col in client.get_collections().collections]

        assert len(collection_names) <= HIST_LEN
        assert "austria_v3" in collection_names  # aliased, should NOT be deleted
        assert "austria_v2" in collection_names  # recent, should NOT be deleted

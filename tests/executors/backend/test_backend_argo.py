...
class TestArgoBackendGeneratedCliExecutor:
    ...
    def test_uses_base_executor_when_prometheus_gateway_set(self, monkeypatch):
        # PrometheusStepExecutor is replaced by the prometheus middleware.
        # The default executor remains BaseStepExecutor regardless of PROMETHEUS_GATEWAY.
    ...
    def test_explicit_executor_without_env(self):
        backend = ArgoBackend(executor=BaseStepExecutor)
        yaml_output = backend.generate_artifact(DummyStep())
        cmd = self._first_wurzel_command(yaml.safe_load(yaml_output))
        assert "-e BaseStepExecutor" in cmd

    def test_uses_base_executor_when_set_in_container_env(self):
        # PrometheusStepExecutor is replaced by the prometheus middleware.
        # Container env with PROMETHEUS_GATEWAY no longer auto-selects a different executor.
    ...
    def test_container_env_uses_base_executor(self, monkeypatch):

...
def test_cut_off_with_returned_cutted_text():
    splitter = SemanticSplitter()
    input_text = "This is a very long long text with many many words that produce a lengthy sentence that is the input for the splitter."

    cut_off_text_10 = splitter._cut_to_tokenlen(text=input_text, token_len=10, return_cutted_text=True)
    cut_off_text_20 = splitter._cut_to_tokenlen(text=input_text, token_len=20, return_cutted_text=True)
    cut_off_text_100 = splitter._cut_to_tokenlen(text=input_text, token_len=100, return_cutted_text=True)  # no cut off
...
def test_cut_off_logging(caplog):
    ...
    assert caplog.records[0].cutted_text == input_text[44:]

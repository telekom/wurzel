...
@pytest.mark.parametrize(
    "cls", [
        leaf_cls, leaf_field_default, leaf_field_defaultfactory
    ])
...
@pytest.mark.parametrize("mapping_default", [
    pytest.param(Field, id="Field_default"),
    pytest.param(lambda x: Field(default_factory=lambda: x),
                 id="Field_defaultfactory"),
...
@pytest.mark.parametrize("env_values,validator", [
...
def test_url_parsing(env, url_class: Type[Union[pyd_c_Url, str]], scheme: Union[Literal["http"], Literal["https"]] ,host: str,port: int,path:str):
    url = pyd_c_Url.build(
        scheme=scheme,
        host=host,
        port=port,

...
def test_multi_step_pipeline_with_history_labels_and_log_counter(tmp_path, env):
    with PrometheusStepExecutor() as ex:
        ...
        assert len(ex.counter_outputs.collect()[0].samples) == 8

        # Check labels
        for sample in ex.counter_outputs.collect()[0].samples:
            assert sample.labels["history_first_step"] == "TstA"

        assert ex.counter_outputs.collect()[0].samples[0].labels["history_last_step"] == "TstA"
        assert ex.counter_outputs.collect()[0].samples[-1].labels["history_last_step"] == "TstD"
...
@pytest.fixture(scope="module")
def test_multi_step_pipeline_with_pipeline_id(tmp_path, env):
    env.set("PROMETHEUS_PIPELINE_ID_ENV_VARIABLES", "WZ_PIPELINE_ID")
...
@pytest.fixture(scope="module")
def test_multi_step_pipeline_with_pipeline_id_from_multiple_envs(tmp_path, env):
    env.set("PROMETHEUS_PIPELINE_ID_ENV_VARIABLES", "WZ_TENANT,WZ_PIPELINE_ID")

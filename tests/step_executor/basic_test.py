...
class TstA2Step(TypedStep[NoSettings, None, MarkdownDataContract]):
    def run(self, inputs: None) -> MarkdownDataContract:
...
class TstCStep(TypedStep[NoSettings, list[MarkdownDataContract], MarkdownDataContract]):
...
def test_chain(tmp_path):
    ...
    with BaseStepExecutor() as ex:
        a = ex.execute_step(TstAStep, None, out_a)
        b = ex.execute_step(TstBStep, (out_a,), out_b)
        c = ex.execute_step(TstCStep, (out_b,), out_c)
    for i in [a, b, c]:
        assert len(i) == 1
...
def test_2_to_1(tmp_path):
    ...
    out_b = tmp_path / "out_b"
    with BaseStepExecutor() as ex:
        ex.execute_step(TstAStep, None, output_dir=out_as[0])
        ex.execute_step(TstA2Step, None, output_dir=out_as[1])
        b = ex.execute_step(TstBStep, out_as, out_b)
    assert len(b) == 2
...
def test_2_to_2_to_1(tmp_path):
    ...
    with BaseStepExecutor() as ex:
        ...
        ex.execute_step(TstA2Step, None, output_dir=out_as[1])
        for out in out_as:
            ex.execute_step(TstBStep, (out,), out_b1)
        c = ex.execute_step(TstCStep, (out_b1,), final)
    assert len(c) == 2
...
def test_2_to_1_to_1(tmp_path):
    ...
    with BaseStepExecutor() as ex:
        ex.execute_step(TstAStep, None, output_dir=out_as[0])
        ex.execute_step(TstA2Step, None, output_dir=out_as[1])
        _ = ex.execute_step(TstBStep, out_as, out_b1)
        c = ex.execute_step(TstCStep, (out_b1,), final)
    assert len(c) == 2
    assert len(list(final.glob("*"))) == 2
    for p in final.glob("*"):

...
class TestArgoBackendSecurityContext:
    ...
    def test_container_security_context(self):
        ...
        for template in container_templates:
            assert probe_fields.isdisjoint(template["container"])
    ...
    def test_node_selector_can_be_cleared_in_workflow(self):
        """Test that default node selector can be cleared from workflow config."""
        config = WorkflowConfig(nodeSelector={})
        backend = ArgoBackend(config=config)
        step = DummyStep()
        yaml_output = backend.generate_artifact(step)
        workflow = yaml.safe_load(yaml_output)
        ...
        spec = workflow.get("spec", {})
        if "workflowSpec" in spec:
            assert "nodeSelector" not in spec["workflowSpec"]
        else:
...
class TestArgoBackendPodSpecPatch:
    ...
    @staticmethod
    def _workflow_spec(workflow: dict) -> dict:
        spec = workflow.get("spec", {})
        return spec.get("workflowSpec", spec)

    def _assert_executor_container_patch(
        self,
        container_patch: dict,
        name: str,
        requests: dict[str, str] | None = None,
        limits: dict[str, str] | None = None,
    ) -> None:
        expected_requests = requests or {"cpu": "100m", "memory": "128Mi"}
        expected_limits = limits or {"memory": "512Mi"}

        assert container_patch["name"] == name
        assert self.probe_fields.isdisjoint(container_patch)
        assert container_patch["resources"]["requests"] == expected_requests
        assert container_patch["resources"]["limits"] == expected_limits
        assert "cpu" not in container_patch["resources"]["limits"]
        assert container_patch["securityContext"] == {
            "allowPrivilegeEscalation": False,
    ...
    def test_default_pod_spec_patch_covers_argo_executor_containers(self):
        """Default podSpecPatch patches Argo's injected executor containers."""
        backend = ArgoBackend()
        pod_spec_patch = yaml.safe_load(backend._build_pod_spec_patch())

        self._assert_executor_container_patch(pod_spec_patch["containers"][0], "wait")
        self._assert_executor_container_patch(pod_spec_patch["initContainers"][0], "init")

    def test_default_pod_spec_patch_is_rendered_into_workflow(self):
        """Generated workflows include the default executor podSpecPatch."""
        backend = ArgoBackend()
        workflow = yaml.safe_load(backend.generate_artifact(DummyStep()))
        workflow_spec = self._workflow_spec(workflow)
        pod_spec_patch = yaml.safe_load(workflow_spec["podSpecPatch"])

        self._assert_executor_container_patch(pod_spec_patch["containers"][0], "wait")
        self._assert_executor_container_patch(pod_spec_patch["initContainers"][0], "init")

    def test_default_pod_spec_patch_uses_configured_resources_without_cpu_limit(self):
        """Executor resources follow workflow resources but never include a CPU limit."""
        config = WorkflowConfig(
            container=ContainerConfig(
        ...
        limits = {"memory": "1Gi"}
        self._assert_executor_container_patch(pod_spec_patch["containers"][0], "wait", requests, limits)
        self._assert_executor_container_patch(pod_spec_patch["initContainers"][0], "init", requests, limits)

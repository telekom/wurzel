...
def step_env_encapsulation(step_cls: Type[TypedStep]):
    ...
    if step.settings_class != NoneType:
        ...
        log.info("Running with encapsulated settings", extra={
            'settings': inner_settings.model_dump(mode="json")
        })
    try:
        yield
    finally:
...
class BaseStepExecutor():
    ...
    def store(
            ...
            step: TypedStep,
            hist: History,
            obj: Union[PanderaDataFrameModel, PydanticModel], path: PathToFolderWithBaseModels)->Path:
        """Store step result
    ...
    def load(
        ...
        for p in path.glob("*"):
            ...
            yield (data, History('.'.join(p.name.split(".")[:-1]), step)), time.time()-start
    def __load_data(
            self,
            step: TypedStep,
            inputs: set[Union[PydanticModel, patyp.DataFrame[PanderaDataFrameModel], PathToFolderWithBaseModels]]
            )  -> Generator[tuple[
                tuple[
                    Union[PydanticModel, patyp.DataFrame[PanderaDataFrameModel], list[PydanticModel], None],
                    History],
                float], Any, NoneType]:
    ...
    def __execute_step(
        ...
        try:
            for (inpt, history), load_time in self.__load_data(step, inputs):
                ...
                log.info(f"Start: {step_cls.__name__}.run({history[:-1]}) -> {output_path}",)
                history: History
                run_start = time.time()
                ctx = copy_context()
                ...
                log.info(f"Finished: {step_cls.__name__}.run({history[:-1]}) -> {output_path}",
                    extra=report.model_dump())
                yield res, report
            log.info(f"{step_cls.__name__}.finalize()")
            if not was_called_once:
                log.error("Run was never executed", extra={
            ...
            raise ContractFailedException(
                f"{inputs} does not conform the data contract of {step.input_model_class.__name__}") from err

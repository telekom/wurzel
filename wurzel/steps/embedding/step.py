...
class EmbeddingStep(SimpleSplitterStep,
    ...
    def __init__(self) -> None:
        ...
        self.embedding = EmbeddingStep._select_embedding(
            self.settings.API
        )
        ...
        self.splitter = SemanticSplitter(token_limit=self.settings.TOKEN_COUNT_MAX,
    ...
    def run(self, inpt: list[MarkdownDataContract]) -> DataFrame[EmbeddingResult]:
        ...
        for row in tqdm(splitted_md_rows, desc="Calculate Embeddings"):
            try:
                ...
                log.warning(f"Skipped because EmbeddingAPIException: {err.message}", extra={"markdown":str(row)})
                failed +=1
        if failed:
        ...
        return DataFrame[EmbeddingResult](DataFrame[EmbeddingResult](rows).drop_duplicates(subset=['text']))
    ...
    def _replace_link(cls,text:str):
        ...
        links = sorted(re.findall(url_extract_pattern, text),
